...
An R client for the [MoSPI Microdata Portal](https://microdata.gov.in).
Downloads unit-level survey data published by India’s Ministry of
...
Get an API key from [microdata.gov.in](https://microdata.gov.in), then
add it to `~/.Renviron`:
...
Failed requests retry up to 4 times with exponential backoff on 401,
429, and 5xx.
